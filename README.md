...
## Description
[cite_start]This repository contains the deliverables for Week 1 of the Mobile Robotics lab[cite: 1, 2, 45]. [cite_start]The primary objectives were to set up a ROS 2 Humble workspace on Ubuntu, learn fundamental Linux navigation, and create our first  custom Python package (`my_first_pkg`)[cite: 2, 7, 8]. [cite_start]The package includes nodes that demonstrate basic logging, file-based execution counting, and ROS 2 parameter handling i personally added a user input in the parameter handling task[cite: 295, 296, 297, 298, 299].

## Commands Used
[cite_start] The following commands were used heavily in this week 1 lab 
* [cite_start]`source /opt/ros/humble/setup.bash`: Sourced the core ROS 2 environment[cite: 143].
* [cite_start]`colcon build`: Compiled the workspace[cite: 152].
* [cite_start]`source install/setup.bash`: Sourced the local workspace overlay[cite: 154].
* [cite_start]`ros2 pkg create --build-type ament_python my_first_pkg`: Created the new Python package[cite: 169].
* [cite_start]`chmod +x <filename>`: Made the Python node scripts executable[cite: 215].
* [cite_start]`ros2 run my_first_pkg <node_name>`: Executed the specific nodes[cite: 234].
* [cite_start]`ros2 run my_first_pkg simple_node_with_parameter --ros-args -p student_name:="Muhammad Rayan"`: Ran a node while passing a custom parameter via the terminal[cite: 234].
